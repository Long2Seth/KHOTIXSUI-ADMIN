...
import { useGetAllEventQuery } from "@/redux/feature/event/Event";
...
export function EventComponent() {
    ...
    const { data, isLoading, error } = useGetAllEventQuery({ pageNumber: currentPage - 1, pageSize: itemsPerPage });
    const dataEvent = data?.content || [];

    const locations = useMemo(() => Array.from(new Set(dataEvent.map(order => order.location))), [dataEvent]);
    const categories = useMemo(() => Array.from(new Set(dataEvent.map(order => order.eventCategory))), [dataEvent]);

    const filteredData = useMemo(() => {
        return dataEvent.filter((item) => {
            ...
            const matchesStatus = selectedStatus === "all" || item.isPublish === (selectedStatus === "true");
    ...
    }, [selectedLocation, selectedStatus, date, eventName, gender, selectedCategory, dataEvent]);
    ...
    const exportToExcel = () => {
        const ws = XLSX.utils.json_to_sheet(dataEvent);
