...
import { useEnableOrganizerMutation, useDisableOrganizerMutation } from "@/redux/feature/user/Organizer";
...
export function ActionOrganizerComponent({ uuid, username, status }: ActionOrganizerComponentProps) {
    const [sta, setSta] = useState(status);
    const [isPopoverOpen, setIsPopoverOpen] = useState(false);
    const [enableOrganizer] = useEnableOrganizerMutation();
    const [disableOrganizer] = useDisableOrganizerMutation();

    const handleStatusChange = () => {
        if (sta === 1) {
            disableOrganizer(uuid)
                .unwrap()
                .then(() => setSta(0))
                .catch(error => console.error("Error updating status:", error));
        } else {
            enableOrganizer(uuid)
                .unwrap()
                .then(() => setSta(1))
                .catch(error => console.error("Error updating status:", error));
        }
