...
export function FeedbackComponent() {
        ...
        const fetchFeedbackData = async () => {
            try {
                const response = await fetch(
                    '/communication/api/v1/feedback?page=0&size=25',
                    {
                        credentials: "include",
                    }
                );
    ...
    const matchDate = (item: FeedbackType) => {
        return date ? format(new Date(item.createdAt), "PPP") === format(date, "PPP") : true;
    };
    ...
    const table = useReactTable<FeedbackType>({
        data: paginatedData,
