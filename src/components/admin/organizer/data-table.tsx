...
import {mockOrganizer} from "@/lib/admin/OrganizerData";
...
export function OrganizerComponent() {
    ...
    const filteredData = useMemo(() => {
        return mockOrganizer.filter((item) => {
    ...
    }, [selectedVerified, selectedStatus, selectedGender]);
    ...
    const exportToExcel = () => {
        const ws = XLSX.utils.json_to_sheet(mockOrganizer);
        ...
        <section className=" w-full flex flex-col ">
            <section>
                <div className="flex justify-between items-start sm:items-center gap-4 mb-5 ">
                    <div>
                        <h1 className="text-title-color text-lg md:text-2xl xl:text-4xl font-bold dark:text-secondary-color-text mb-1 md:mb-2 ">ORGANIZER MANAGEMENT</h1>
            ...
            <section className="w-full bg-white p-10 rounded-[6px] dark:backdrop-blur dark:bg-opacity-5 space-y-4">
                ...
                <section className="w-full flex flex-col items-center gap-2 lg:flex-row ">
                    <Input
                        ...
                        value={(table.getColumn("username")?.getFilterValue() as string) ?? ""} // Ensure this matches the accessorKey
                        onChange={(event) =>
                            table.getColumn("username")?.setFilterValue(event.target.value) // Ensure this matches the accessorKey
                        }
